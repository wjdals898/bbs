...
<body>
		...
		<div class="navbar-header">
	  	...
	  	<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
	  		<ul class="nav navbar-nav">
	  			...
	  			<li class="active"><a href="HOTbbs.jsp">핫 게시판</a></li> 
	  		</ul>
	  	<%
	  		}
	  	%>
	...
	%>
		<div class="row">
			...
			%>
				<a href="HOTbbs.jsp" class="btn btn-primary">목록</a>
				<a onClick="return alert('추천하였습니다')" href="HOTrecommendAction.jsp?bbsID=<%= bbsID %>" class="btn btn-primary">추천</a>
				<a href="HOTupdate.jsp?bbsID=<%= bbsID %>" class="btn btn-primary">수정</a>
				<a onClick="return confirm('정말로 삭제하시겠습니까?')" href="HOTdeleteAction.jsp?bbsID=<%= bbsID %>" class="btn btn-primary">삭제</a>
			<%
				} else {
			%>
				<a href="HOTbbs.jsp" class="btn btn-primary">목록</a>
				...
				<a href="HOTaddFriendAction.jsp?friendID=<%= writerID %>&userID=<%=userID %>&bbsID=<%=bbsID %>" class="btn btn-primary pull-right">친구추가</a> 
			<%
				}
			%>
			<div>
				...
				<tbody>
					...
					<tr>
						...
						<td class="reply-button<%= i %>" style="width:3%; padding-left:0; padding-right:0;">
							<a style="font-size:12px;" href="HOTreplyrecommendAction.jsp?bbsID=<%= bbsID %>&replyID=<%= list.get(i).getReplyID() %>" class="btn btn-secondary">추천</a>
						</td>
						...
						<td style="width:3%; padding-left:0; padding-right:0;">
							<a style="font-size:12px;" onclick="return confirm('정말로 삭제하시겠습니까?')" href="HOTreplydeleteAction.jsp?replyID=<%= list.get(i).getReplyID() %>&bbsID=<%= bbsID %>" class="btn btn-secondary">삭제</a>
						</td>
					</tr>
					<tr id="fixReply<%= i %>" style="display:none; visibility:hidden;">
						...
						<td colspan="5">
							<div>
								<form method="post" action="HOTreplyupdateAction.jsp?bbsID=<%=bbsID %>&replyID=<%=list.get(i).getReplyID() %>">
									<div>
										<textarea id="fixReplyContent<%=i %>" class="form-control" name="replyContent" maxlength="1024" style="height:80px;"></textarea>
									</div>
									<div>
										<a href="HOTview.jsp?bbsID=<%= bbsID %>" class="btn btn-primary pull-right" style="margin:5px;">취소</a>
									</div>
									<div>
					...
					<tr>
						...
						<td style="width:3%; padding-left:0; padding-right:0;">
							<a style="font-size:12px;" onclick="return confirm('정말로 삭제하시겠습니까?')" href="HOTrereplydeleteAction.jsp?rereplyID=<%= reList.get(j).getRereplyID() %>&bbsID=<%= bbsID %>" class="btn btn-secondary">삭제</a>
						</td>
					</tr>
					<tr id="reReplyUpdate<%=j %>" style="display:none; visibility:hidden;">
						...
						<td colspan="5">
							<div>
								<form method="post" action="HOTrereplyupdateAction.jsp?bbsID=<%=bbsID %>&rereplyID=<%=reList.get(j).getRereplyID() %>">
									<div>
					...
					<tr id="rereplywrite<%=i %>" style="display:none; visibility:hidden;"> <!-- style="display:none; visibility:hidden;" -->
						<td></td>
						<td><%= userID %></td>
						<td colspan="5">
							<div>
								<form method="post" action="HOTrereplywriteAction.jsp?bbsID=<%=bbsID %>&replyID=<%=list.get(i).getReplyID() %>">
									<div>
				...
				<div class="container">
					<form method="post" action="HOTreplywriteAction.jsp?bbsID=<%= bbsID %>">
