...
<body>
	<%
		...
		if(replyID == 0){
			...
			script.println("location.href = 'Userview.jsp?bbsID="+request.getParameter("bbsID")+"&replyID="+request.getParameter("vID")+"'");
			script.println("</script>");
		}
		...
		if(!userID.equals(reply.getUserID())){
			...
			if(request.getParameter("replyContent") == null)
			{
				...
				if(result == -1 ){
					...
					PrintWriter script = response.getWriter();
					script.println("<script>");
					script.println("location.href = 'Userview.jsp?bbsID="+request.getParameter("bbsID")+"&replyID="+request.getParameter("replyID")+"'");
					script.println("</script>");		
					}
			}
		}
