...
<body>
		...
		<div class="navbar-header">
	  	...
	  	<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
	  		<ul class="nav navbar-nav">
	  			...
	  			<li class="active"><a href="mybbs.jsp">마이 게시판</a></li> <!-- 개인 게시판 Mybbs.jsp예정 -->
	...
	<div class="container">
		<div class="row">
			...
			<a href="mybbs.jsp" class="btn btn-primary">목록</a>
			<a onClick="return alert('추천하였습니다')" href="mbrecommendAction.jsp?bbsID=<%= bbsID %>" class="btn btn-primary">추천</a>
			<%
				if(userID != null && userID.equals(bbs.getUserID())) {
			%>
				<a href="mbupdate.jsp?bbsID=<%= bbsID %>" class="btn btn-primary">수정</a>
				<a onClick="return confirm('정말로 삭제하시겠습니까?')" href="mbdeleteAction.jsp?bbsID=<%= bbsID %>" class="btn btn-primary">삭제</a>
			<%
				}
			%>
			<div>
				...
				<tbody>
					...
					<tr>
						...
						<td class="reply-button<%= i %>" style="width:3%; padding-left:0; padding-right:0;">
							<a style="font-size:12px;" href="mbreplyrecommendAction.jsp?bbsID=<%= bbsID %>&replyID=<%= list.get(i).getReplyID() %>" class="btn btn-secondary">추천</a>
						</td>
						...
						<td style="width:3%; padding-left:0; padding-right:0;">
							<a style="font-size:12px;" onclick="return confirm('정말로 삭제하시겠습니까?')" href="mbreplydeleteAction.jsp?replyID=<%= list.get(i).getReplyID() %>&bbsID=<%= bbsID %>" class="btn btn-secondary">삭제</a>
						</td>
					</tr>
					<tr id="fixReply<%= i %>" style="display:none; visibility:hidden;">
						...
						<td colspan="5">
							<div>
								<form method="post" action="mbreplyupdateAction.jsp?bbsID=<%=bbsID %>&replyID=<%=list.get(i).getReplyID() %>">
									<div>
										<textarea id="fixReplyContent<%=i %>" class="form-control" name="replyContent" maxlength="1024" style="height:80px;"></textarea>
									</div>
									<div>
										<a href="mbview.jsp?bbsID=<%= bbsID %>" class="btn btn-primary pull-right" style="margin:5px;">취소</a>
									</div>
									<div>
					...
					<tr>
						...
						<td style="width:3%; padding-left:0; padding-right:0;">
							<a style="font-size:12px;" onclick="return confirm('정말로 삭제하시겠습니까?')" href="mbrereplydeleteAction.jsp?rereplyID=<%= reList.get(j).getRereplyID() %>&bbsID=<%= bbsID %>" class="btn btn-secondary">삭제</a>
						</td>
					</tr>
					<tr id="reReplyUpdate<%=j %>" style="display:none; visibility:hidden;">
						...
						<td colspan="5">
							<div>
								<form method="post" action="mbrereplyupdateAction.jsp?bbsID=<%=bbsID %>&rereplyID=<%=reList.get(j).getRereplyID() %>">
									<div>
					...
					<tr id="rereplywrite<%=i %>" style="display:none; visibility:hidden;"> <!-- style="display:none; visibility:hidden;" -->
						...
						<td colspan="5">
							<div>
								<form method="post" action="mbrereplywriteAction.jsp?bbsID=<%=bbsID %>&replyID=<%=list.get(i).getReplyID() %>">
									<div>
				...
				<div class="container">
					<form method="post" action="mbreplywriteAction.jsp?bbsID=<%= bbsID %>">
