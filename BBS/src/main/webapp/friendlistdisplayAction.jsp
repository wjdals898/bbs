...
<body>
	<%
		...
		if(friendID == ""){
			...
			script.println("location.href = 'friendhidelist.jsp'");
			script.println("</script>");
		}
		...
		if(!userID.equals(friend.getUserID())){
			PrintWriter script = response.getWriter();
			script.println("<script>");
			script.println("alert('숨길 수 없습니다.')");
			script.println("location.href = 'main.jsp'");
			script.println("</script>");
		} else {
			FriendDAO friendDAO = new FriendDAO();
			int result = friendDAO.display(friendID);
			if(result == -1 ){
				PrintWriter script = response.getWriter();
				script.println("<script>");
				script.println("alert('친구 숨김취소에 실패했습니다')");
